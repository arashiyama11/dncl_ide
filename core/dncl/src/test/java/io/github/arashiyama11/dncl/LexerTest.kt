...
表示する("次の工芸品の担当は部員", tantou, "です")"""
        ...
        assertEquals(
            "NewLineIdentifier(literal=Akibi)AssignBracketOpenInt(literal=5)CommaInt(literal=3)CommaInt(literal=4)BracketCloseNewLineIdentifier(literal=buinsu)AssignInt(literal=3)NewLineIdentifier(literal=tantou)AssignInt(literal=1)NewLineIdentifier(literal=buin)Japanese(literal=を)Int(literal=2)Japanese(literal=から)Identifier(literal=buinsu)Japanese(literal=まで)Int(literal=1)Japanese(literal=ずつ増やしながら繰り返す)ColonNewLineIndent(depth=2, literal=Indent(2))IfIdentifier(literal=Akibi)BracketOpenIdentifier(literal=buin)BracketCloseLessThanIdentifier(literal=Akibi)BracketOpenIdentifier(literal=tantou)BracketCloseThenColonNewLineIndent(depth=4, literal=Indent(4))Identifier(literal=tantou)AssignIdentifier(literal=buin)NewLineJapanese(literal=表示する)ParenOpenString(literal=次の工芸品の担当は部員)CommaIdentifier(literal=tantou)CommaString(literal=です)ParenCloseEOF",
            result
        )
    }
